...
function getRoleBadge(role: string, position?: string) {
    switch (role) {
        case 'officer':
            return { text: position || 'Valde', color: 'bg-blue-100 text-blue-700' };
        case 'member':
            return { text: 'Dalībnieks', color: 'bg-green-100 text-green-700' };
        case 'ubo':
            return { text: 'PLG', color: 'bg-yellow-100 text-yellow-700' };
        default:
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (!person) return { title: "Persona nav atrasta" };
    return {
        title: `${person.full_name} - Personas Profils`,
        description: `Biznesa profils: ${person.full_name}. ${person.kpi.active_companies_count} aktīvie uzņēmumi.`,
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                            ...
                            {person.risk_badges.tax_debt && (
                                <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-yellow-100 text-yellow-800 border border-yellow-300">
                                    ⚠️ Saistīts ar nodokļu parādiem
                                </span>
                            )}
                        </div>
                    )}
                    ...
                    <div className="flex flex-wrap gap-6 text-sm text-gray-600 mb-4">
                        <span className="flex items-center gap-1">
                            ...
                            Personas kods: {person.person_code_masked}
                        </span>
                        {person.birth_date && (
                            <span className="flex items-center gap-1">
                                <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                Dzimšanas datums: {new Date(person.birth_date).toLocaleDateString('lv-LV')}
                            </span>
                        )}
                        <span className="flex items-center gap-1">
                            <svg className="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            Valstspiederība: {person.nationality === 'LV' ? 'Latvija' : person.nationality}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2">
                        <div className="bg-white rounded-lg shadow-md overflow-hidden">
                            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                                <h2 className="text-xl font-bold text-primary">Saistītie Uzņēmumi</h2>
                            </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Loma</th>
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Daļas</th>
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Apgrozījums</th>
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Statuss</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {activeCompanies.length === 0 ? (
                                            ...
                                            activeCompanies.map((company: any) => {
                                                const badge = getRoleBadge(company.role, company.position);
                                                return (
                                                    <tr key={company.regcode} className="hover:bg-gray-50">
                                                        <td className="px-6 py-4">
                                                            ...
                                                            <div className="text-xs text-gray-500">Reģ. Nr. {company.regcode}</div>
                                                        </td>
                                                        <td className="px-6 py-4">
                    ...
                    <div className="lg:col-span-1">
                        <div className="bg-white rounded-lg shadow-md overflow-hidden">
                            <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                                <h2 className="text-lg font-bold text-primary">Sadarbības Tīkls</h2>
                                <p className="text-xs text-gray-500 mt-1">Personas, ar kurām kopā uzņēmumos</p>
                            </div>
                            <div className="p-4">
