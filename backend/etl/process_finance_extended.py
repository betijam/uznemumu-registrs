"""
Extended Financial Data Processing with Cash Flow, Accounts Receivable, and Labour Expenses
This extends the existing process_finance.py to include additional fields for advanced financial analysis.
"""
import pandas as pd
import numpy as np
import logging
...
def process_finance_extended(statements_path: str, balance_path: str, income_path: str):
    """
    EXTENDED Process financial data with Cash Flow, Accounts Receivable, and Labour Expenses
    Uses CHUNKED LOADING to prevent memory issues.
    """
    logger.info("🔄 Processing Finance with EXTENDED FIELDS (Memory-Optimized)...")
    ...
    try:
        ...
        try:
            ...
            for inc_chunk in pd.read_csv(income_path, sep=';', dtype=str, chunksize=CHUNK_SIZE):
                ...
                # EXTENDED Column mapping for income statement
                income_mapping = {
                    ...
                    'darba_samaksa': 'by_nature_labour_expenses'
                }
        ...
        try:
            for bal_chunk in pd.read_csv(balance_path, sep=';', dtype=str, chunksize=CHUNK_SIZE):
                ...
                # EXTENDED Column mapping for balance sheet
                balance_mapping = {
                    ...
                    # CASH FLOW FIELDS (if in balance sheet file)
                ...
                # Add income data
                all_income_cols = [
                    'turnover', 'profit', 'interest_expenses', 'depreciation_expenses',
                    'provision_for_income_taxes', 'by_nature_labour_expenses'  # NEW
                ]
                ...
                final_cols = [
                    ...
                    'accounts_receivable',  # NEW
                    # Cash Flow
                    'cfo_im_net_operating_cash_flow', 'cfo_im_income_taxes_paid',  # NEW
                    'cfi_acquisition_of_fixed_assets_intangible_assets', 'cff_net_financing_cash_flow',  # NEW
                    # Calculated Ratios
...
_to_db(df_merged, 'financial_reports', truncate=is_first_chunk)
        ...
        logger.info(f"✅ EXTENDED Financial data processing complete: {total_loaded} total records loaded")
            
    except Exception as e:
        logger.error(f"Error processing extended finance: {e}")
        raise
